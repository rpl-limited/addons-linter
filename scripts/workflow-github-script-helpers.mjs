...
export async function findExistingIssue({ github, context, firefox_version }) {
  const issues = await github.request('GET /repos/{owner}/{repo}/issues', {
    ...
    q: `Import Firefox API Schema for ${firefox_version}`,
  });
...
export async function createIssue({ github, context, importState }) {
  ...
  const title = `Import Firefox API Schema for ${firefox_version}`;
  const body = [
    `This issue is tracking importing Firefox API Schemas for Firefox ${firefox_version}.`,
    '',
...
export async function createPull({ github, context, importState, issue }) {
  ...
  if (!beta_build_number) {
    shell.echo('No pull request created. Import not related to a mozilla-central beta branch');
    return null;
  }
  if (has_remote_branch_changes) {
    shell.echo(`No pull request created. Found changes applied to the remote ${branch_name}`);
    return null;
  }
...
function hasPendingSchemaChanges(firefox_version_display) {
  shell.echo(`Check for changes from Importing Firefox ${firefox_version_display} API Schema...`);
...
async function hasRemoteBranchChanges({ github, context, branch_name }) {
  shell.echo(`Check for existing remote changes in the branch ${branch_name}...`);

  const githubRemoteBranchResult = await github.rest.repos.getBranch({
    owner: context.repo.owner,
    repo: context.repo.repo,
    branch: branch_name,
  }).catch((err) => err);
  ...
  shell.exec(`git fetch origin ${branch_name}`, { silent: true });
  shell.exec(`git fetch origin ${MAIN_BRANCH}`, { silent: true });

  const remoteBranchExists = shell
    .exec(`git branch -r --no-merged`, { silent: true })
    .stdout.includes(`origin/${branch_name}`);

  if (!remoteBranchExists) {
    throw new Error(`Unexpected issue with fetching remote branch ${branch_name}`);
  }

  const remoteBranchForkPoint = shell
    .exec(
      `git merge-base --fork-point remotes/origin/${MAIN_BRANCH} remotes/origin/${branch_name}`,
      { silent: true }
    )
    .stdout.trim();
  const logs = shell
    .exec(
      `git log --pretty=oneline -r ${remoteBranchForkPoint}...remotes/origin/${branch_name}`,
      { silent: true }
    )
    .stdout.split('\n');
  if (logs.length > 1) {
    console.log(`Found remote commits in branch ${branch_name}:\n`, logs.join('\n'));
    return true;
  }
  return false;
}
...
export async function getImportResultState({ github, context, workflowInputs }) {
  ...
  const tests = workflowInputs['run-tests']
    ? runTests()
    : undefined;
  ...
  const resultState = {
    ...
    beta_build_number: beta_build_number ? parseInt(beta_build_number, 10) : undefined,
    nightly_build_number: nightly_build_number ? parseInt(nightly_build_number, 10) : undefined,
    branch_name,
    current_date,
    has_pending_changes,
    has_remote_branch_changes,
  ...
  shell.echo(`Firefox API Schema import result state: ${JSON.stringify(resultState, null, 2)}`);
  return resultState;
}

export function createBranchAndCommit({ branch_name, version_display, current_date }) {
  ...
  shell.exec(
    `git commit -m "feat: Imported Firefox API Schema from Firefox ${version_display} ($GITHUB_JOB ${current_date})"`
  );
